...
dataVectorEdgeUsed=[];
for i=1:1:AlgNum
    dataNodeUsed=load([FileAbsolutePath,'Data\\Utilization_SubNode_Accumulate',strtrim(AlgName(i,:)),'.txt']);
    dataEdgeUsed=load([FileAbsolutePath,'Data\\Utilization_SubEdge_Accumulate',strtrim(AlgName(i,:)),'.txt']);
    
    dataNode=load([FileAbsolutePath,'Data\\Cost_NodeCp_Accumulate',strtrim(AlgName(i,:)),'.txt']);
    dataEdge=load([FileAbsolutePath,'Data\\Cost_EdgeBw_Accumulate',strtrim(AlgName(i,:)),'.txt']);
...
title('SubstrateNewtork Accumulate Utilization');
...
saveas(gcf,[FileAbsolutePath,'Data\\Fig','\\UtilizationAccumulateSubstrateNetwork','.eps']);
saveas(gcf,[FileAbsolutePath,'Data\\Fig','\\UtilizationAccumulateSubstrateNetwork','.jpg']);
