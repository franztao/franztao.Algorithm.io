...
for i=1:1:AlgNum
    dataCostNodeUsed=load([FileAbsolutePath,'Data\\Utilization_SubNode_',strtrim(AlgName(i,:)),'.txt']);
    dataCostEdgeUsed=load([FileAbsolutePath,'Data\\Utilization_SubEdge_',strtrim(AlgName(i,:)),'.txt']);
    
    dataCostNode=load([FileAbsolutePath,'Data\\Cost_NodeCp_',strtrim(AlgName(i,:)),'.txt']);
    dataCostEdge=load([FileAbsolutePath,'Data\\Cost_NdgeBw_',strtrim(AlgName(i,:)),'.txt']);
...
for i=1:1:AlgNum
    for j=1:1:c
        ...
        experimentCostEdgeUsed(i,mod(j-1,ExperimentPicturePlotNumber)+1)= experimentCostEdgeUsed(i,mod(j-1,ExperimentPicturePlotNumber)+1)+dataCostVectorEdgeUsed(i,j);
    end
end
...
for i=1:1:AlgNum
    dataRevenueNodeUsed=load([FileAbsolutePath,'Data\\Utilization_VirNode_',strtrim(AlgName(i,:)),'.txt']);
    dataRevenueEdgeUsed=load([FileAbsolutePath,'Data\\Utilization_VirEdge_',strtrim(AlgName(i,:)),'.txt']);
    
    dataRevenueNode=load([FileAbsolutePath,'Data\\Revenue_NodeCp_',strtrim(AlgName(i,:)),'.txt']);
    dataRevenueEdge=load([FileAbsolutePath,'Data\\Revenue_EdgeBw_',strtrim(AlgName(i,:)),'.txt']);
...
saveas(gcf,[FileAbsolutePath,'Data\\Fig','\\UtilizationSubVir2VirNet','.eps']);
saveas(gcf,[FileAbsolutePath,'Data\\Fig','\\UtilizationSubVir2VirNet','.jpg']);
