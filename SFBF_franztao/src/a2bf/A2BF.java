...
public class A2BF {
	...
	public A2BF(int hashfunctionnum, HashGenerationMatrice[] hashgenmatrice2, int cachesize) {
	...
	public boolean inserteverykey(InputData a) {
		...
		if (query(a, A2BFBloomFilterGroup.activebloomfiltervalue[active])) {
			...
			if (query(a, A2BFBloomFilterGroup.activebloomfiltervalue[(active + 1) % 2])) {
				result = true;
			} else {
				result = false;
				...
				if (this.activesize[active] > this.n) {
					...
					this.reset_num++;
					insert(a, A2BFBloomFilterGroup.activebloomfiltervalue[active]);
					// System.out.println(this.reset_num+"-----"+this.activesize[active]+"
					// "+this.n);
				}
			}
		}
