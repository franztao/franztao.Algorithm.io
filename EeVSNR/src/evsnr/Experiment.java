...
public class Experiment {
  ...
  private void runComparableAlgorithm(int experimentTimes) {
    ...
    for (int alg = 0; alg < algorithms.size(); alg++) {
      ...
      for (int time = 0; time <= Parameter.SubstrateNewtorkRunTimeInterval; time++) {
        ...
        algorithms.get(alg).releaseResource(false);
        if ((0 == (time % Parameter.VNRequestsDuration)) && (Math.random() < Parameter.requestAppearProbability)) {
  ...
  private void generateComparableAlgorithm(VirtualNetworkParameter vnp2) {
    ...
    try {
      ...
      alg = new Algorithm();
      alg.setParameter("sn_FI_NoShared", sn_FI_NoShared, false, Parameter.FailureIndependent, false);
      alg.setSequence(Parameter.Ran);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FI_Shared_Min = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FI_Shared_Min", sn_FI_Shared_Min, false, Parameter.FailureIndependent, true);
      alg.setSequence(Parameter.Min);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FI_NoShared_Min = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FI_NoShared_Min", sn_FI_NoShared_Min, false, Parameter.FailureIndependent, false);
      alg.setSequence(Parameter.Min);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FD_EVSNR_Ran_Shared = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FD_EVSNR_Ran_Shared", sn_FD_EVSNR_Ran_Shared, false, Parameter.FailureDependent, true);
      alg.setSequence(Parameter.Ran);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FD_EVSNR_Ran_NoShared = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FD_EVSNR_Ran_NoShared", sn_FD_EVSNR_Ran_NoShared, false, Parameter.FailureDependent, false);
      alg.setSequence(Parameter.Ran);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FD_EVSNR_Min_Shared = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FD_EVSNR_Min_Shared", sn_FD_EVSNR_Min_Shared, false, Parameter.FailureDependent, true);
      alg.setSequence(Parameter.Min);
      this.algorithms.addElement(alg);

      SubstrateNetwork sn_FD_EVSNR_Min_NoShared = (SubstrateNetwork) this.basicSubstrateNework.clone();
      alg = new Algorithm();
      alg.setParameter("sn_FD_EVSNR_Min_NoShared", sn_FD_EVSNR_Min_NoShared, false, Parameter.FailureDependent, false);
      alg.setSequence(Parameter.Min);
      this.algorithms.addElement(alg);
